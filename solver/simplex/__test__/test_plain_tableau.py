...
class Test(unittest.TestCase):
    ...
    def test_objective(self):
        var_names = self.tableau.var_names
        coeffs = self.tableau.objective_function

        self.assertEqual(var_names, ['x_1', 'x_2', '_s_1', '_s_2', '_a_1'])
        self.assertEqual(list(coeffs), [-5., -10., 0., 0., 1000.])

    def test_model_vars(self):
        self.assertEqual(len(self.tableau.model_vars), 2)

    def test_variable_count(self):
        self.assertEqual(len(self.tableau.var_names), 5)

    def test_intermediate_solutino(self):
        sol = self.tableau.var_result()
